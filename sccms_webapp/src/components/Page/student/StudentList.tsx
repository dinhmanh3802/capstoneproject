...
const StudentList = ({
    ...
    const handleStatusChange = (selectedOption: any, row: any) => {
        if (selectedOption.value === SD_ProcessStatus.DropOut) {
            setCurrentRow(row)
            setShowConfirmModal(true) // Hiển thị popup nếu chọn DropOut
        } else {
            updateStudentStatus(selectedOption.value, row, "")
        }
    }

    const updateStudentStatus = async (status: SD_ProcessStatus, row: Application, reason: string) => {
        const studentApplicationUpdate = {
            Ids: [row.id],
            Status: status,
            Note: reason || row.note, // Ghi chú thêm lý do nếu cần
    ...
    const handleConfirmDropOut = () => {
        if (currentRow) {
            updateStudentStatus(SD_ProcessStatus.DropOut, currentRow, dropOutReason)
    ...
    const columns: TableColumn<Application>[] = [
            ...
            ? [
                  {
                      ...
                      selector: (row: Application) => {
                          if (
                              row.student &&
                              Array.isArray(row.student.studentGroups) &&
                              row.student.studentGroups.length > 0
                          ) {
        ...
        {
            name: "Trạng thái",
            minWidth: isGroupDetail ? "11rem" : "15rem",
        ...
        },
        {
            ...
            cell: (row) => (
                <div>
                    <Link to={`/students/${row?.id}/course/${row?.courseId}`} className="me-2 fs-3">
                        <OverlayTrigger
                            placement="top"
    ...
    if (courseLoading) return <div>Đang tải dữ liệu...</div>
    return (
        <div className="card">
            ...
            <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)}>
                ...
                <Modal.Body>
                    <p>
                        Xác nhận khóa sinh <strong>{currentRow?.student?.fullName}</strong> rời khỏi khóa tu
                    </p>
                    <div>
                        <label>Lý do:</label>
                        <input
